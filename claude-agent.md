...
- Nếu user muốn sửa design theo mô tả tự do (không theo checklist từ audit), không dùng skill này.
- Nếu user muốn kiểm tra design-system token mapping hoặc reconnect design vào design system, không dùng skill này.
...
- `bao-cao.html` **không** khớp shell `html-template.md` (thiếu/khác khối `<style>` chuẩn, hoặc tự thêm CSS ngoài template).
...
7. **Chạy Measurement Scripts** (ngay sau khi tên đã chuẩn hóa, trước khi vào P1):
   ...
   g. **UX Writing Lint** (`scripts/ux-writing-lint.md`):
      - Quét microcopy/CTA/error message theo rule, xuất bảng nodeId + rule violated
      - Dùng để tăng chất lượng UX Writing và giảm bỏ sót UX-07

   h. **Friction Scanner** (`scripts/friction-scanner.md`):
...
12. **Điền section "Chi phí thực hiện audit (token usage)"** trong báo cáo:
    ...
    - Đối chiếu phase budget (P0 5–10K, P1 1.5–2K, P2 1.5K/lens, P3 4–6K) — nếu lệch >50%, ghi note giải thích.
    - Ghi rõ method: ước tính dựa scratchpad + budget, không phải API billing exact (sai số ±15%).
13. **Xuất 2 file**: `bao-cao.md` + `bao-cao.html` (ảnh base64), thư mục `~/Downloads/audit-report-<screen>-<YYYY-MM-DD>/`. **Tuỳ chọn ổn định UI:** sau khi có `bao-cao.md`, chạy `npm run render-report -- <path>/bao-cao.md` trong repo skill (dùng `scripts/report-shell.html`).
...
- **Figma fetch fail** → DỪNG, không "proceed degraded". Phân loại lỗi và báo user theo 1 trong 4 trường hợp:
  ...
  1. **MCP type sai** — agent yêu cầu mở Figma desktop / bật Dev Mode → bạn đang dùng **Figma Desktop/Dev Mode MCP**. Nếu muốn luồng official khuyến nghị, hãy dùng **Figma Remote MCP** (OAuth) theo doc Figma:
  ...
  2. **Không đủ quyền** — `403 / permission denied`. File thuộc team khác → mời **tài khoản Figma đã Authenticate OAuth** vào file/team, hoặc Authenticate lại bằng tài khoản có quyền.

  3. **URL thiếu `node-id`** — agent đọc toàn file thay vì 1 frame. Yêu cầu user copy link từ frame cụ thể (URL có `?node-id=...`).

  4. **Node không tồn tại** — node-id đúng format nhưng MCP không tìm thấy (frame đã bị xoá / đổi ID). Yêu cầu user copy link mới.
...
Áp dụng Cơ chế hỏi bổ sung. Nếu user không trả lời hoặc yêu cầu bỏ qua, suy luận JTBD từ visual content trên screen và ghi rõ "JTBD context được suy luận" trong báo cáo. Trục UC sẽ có confidence thấp hơn.
...
- Quy tắc gate + công thức: `gate-rules.md`
- Checklist 4 trục: `checklist.md`
- Bộ tiêu chuẩn heuristic: `heuristics.md`
- JTBD framework cho audit: `jtbd-framework.md`
- Template báo cáo: `report-template.md`
- Hướng dẫn xuất HTML: `html-template.md`
- Phạm vi & tham chiếu: `PHAM-VI-TIEU-CHI-VA-THAM-CHIEU.md`
