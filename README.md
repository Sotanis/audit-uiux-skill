...
|------|---------------|------|
| **Cursor** | `SKILL.md` + 8 file KB + `scripts/` | `~/.cursor/skills/audit-uiux/` |
| **Claude Code** | `claude-agent.md` (→ đổi tên thành `audit-uiux.md`) + 8 file KB + `scripts/` | `~/.claude/agents/` |
...
Thư mục `scripts/` chứa 10 playbook đo/scan (contrast, tap target, spacing, UX state/flow/destructive, naming, writing, friction, metadata-stat) — `install.sh` đã tự copy.
...
| `scripts/ux-state-scanner.md` | Quét state UX (loading/empty/error/success) theo evidence |
| `scripts/destructive-action-scanner.md` | Quét destructive actions + confirm/undo evidence |
| `scripts/ux-flow-scanner.md` | Quét flow: dead-end/escape hatch/continuity (evidence-first) |
| `scripts/naming-scanner.md` | Chuẩn hoá tên layer/state/CTA để giảm miss khi audit |
| `scripts/contrast-checker.md` | Đo contrast WCAG (Hard Gate H1 — measured) |
| `scripts/tap-target-checker.md` | Đo tap target (Hard Gate H2 — measured) |
| `scripts/spacing-scanner.md` | Quét spacing scale + magic numbers (UI-03 — measured) |
| `scripts/ux-writing-lint.md` | Lint microcopy/CTA/error theo rule (UX-07) |
| `scripts/friction-scanner.md` | Đo friction proxy (UX-02 — cognitive load) |
| `scripts/metadata-stat-counter.md` | Đếm tỷ lệ default name / hardcoded color / detached / auto-layout / primary CTA (Hard Gate H3, H8, H9, H10, H11 — measured) |
