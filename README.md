...
**Lưu ý về thư mục `scripts/`:**
- `install.sh` hiện copy **SKILL + KB files**. Các file trong `scripts/` là “playbook” hỗ trợ đo/scan (contrast, tap target, spacing, UX state/flow/destructive…).
- Để đảm bảo bạn có đủ `scripts/`, hãy **clone repo** hoặc **Download ZIP** rồi copy cả thư mục `scripts/` vào cùng thư mục cài đặt:
  - Cursor: `~/.cursor/skills/audit-uiux/scripts/`
  - Claude Code: nếu bạn muốn tham chiếu scripts khi chạy, đặt cạnh agent files trong `~/.claude/agents/scripts/` hoặc giữ trong repo và mở cùng lúc.
