...
> Section này BẮT BUỘC có trong mọi báo cáo. Mục đích: người đọc thấy ngay item nào pass, item nào fail, vì sao — không chỉ thấy con số %.

### Trục UI (Craft) — [pass]/11 pass = [x]%

| ID | Tiêu chí | Ngưỡng pass | Kết quả thực tế | Pass? | Method | Ghi chú cụ thể |
|----|----------|-------------|-----------------|-------|--------|----------------|
| UI-01 | Token color | ≥90% bind variable | [x]% ([n]/[total] element) | ✅/❌ | measured | [VD: 45/50 element bind variable, 5 hardcoded hex] |
| UI-02 | Token typography | ≥90% text style | [x]% | ✅/❌ | measured | |
| UI-03 | Spacing scale | 100% ∈ {4,8,12,16,24,32,48} | [x]% | ✅/❌ | measured | [VD: 2 magic number: 13px, 17px — tại node X, Y] |
| UI-04 | Type hierarchy | ≤6 distinct font size | [n] sizes | ✅/❌ | measured | [VD: 5 sizes — 12/14/16/20/24] |
| UI-05 | Border radius | ≤3 distinct values | [n] values | ✅/❌ | measured | [VD: 4/8/12 — OK] |
| UI-06 | Color palette | ≤7 distinct color | [n] colors | ✅/❌ | measured | |
| UI-07 | Icon consistency | Cùng style, size chuẩn | [mô tả] | ✅/❌ | inferred | [VD: 2 icon outline + 1 filled → inconsistent] |
| UI-08 | Auto-layout | ≥80% container | [x]% | ✅/❌ | measured | |
| UI-09 | Constraint set | 100% có constraint | [x]% | ✅/❌ | measured | |
| UI-10 | State coverage button | ≥4/5 state | [n]/5 | ✅/❌ | inferred | [VD: thiếu focused, disabled] |
| UI-11 | State coverage input | ≥4/5 state | [n]/5 | ✅/❌ | inferred | [VD: thiếu error state] |

### Trục UX (Usability) — [pass]/9 pass = [x]% ⚠️ toàn inferred ±15%

| ID | Tiêu chí | Ngưỡng pass | Kết quả thực tế | Pass? | Method | Ghi chú cụ thể |
|----|----------|-------------|-----------------|-------|--------|----------------|
| UX-02 | Cognitive load | ≤12 trên viewport | [score] | ✅/❌ | inferred | [VD: 8 interactive + 3 groups×0.5 + 1 decision×2 = 11.5] |
| UX-05 | Loading state | 100% async có loading | [x]% | ✅/❌ | inferred | [VD: áp dụng filter không có loading → FAIL] |
| UX-06 | Empty state guidance | 100% có icon+msg+CTA | [x]% | ✅/❌ | inferred | [VD: filter 0 kết quả không có empty state] |
| UX-07 | Error message chất lượng | Nói rõ vấn đề + cách sửa | [mô tả] | ✅/❌ | inferred | |
| UX-08 | Dead-end | 0 dead-end | [n] dead-end | ✅/❌ | inferred | |
| UX-09 | Escape hatch | Mọi luồng có thoát/hủy | [mô tả] | ✅/❌ | inferred | |
| UX-10 | Confirmation destructive | 100% có confirm/undo | [x]% | ✅/❌ | inferred | [VD: "Đặt lại" không có confirm → FAIL] |
| UX-11 | Consistency platform | Tuân HIG/Material | [mô tả] | ✅/❌ | inferred | [VD: nút × ở top-left thay vì top-right → vi phạm HIG] |
| UX-12 | Recognition over recall | Info visible | [mô tả] | ✅/❌ | inferred | |

### Trục Nghiệp vụ (Business Logic) — [pass]/10 pass = [x]% ⚠️ toàn inferred ±15%

| ID | Tiêu chí | Ngưỡng pass | Kết quả thực tế | Pass? | Method | Ghi chú cụ thể |
|----|----------|-------------|-----------------|-------|--------|----------------|
| NV-01 | Happy path | 100% job step có UI | [x]% | ✅/❌ | inferred | [Liệt kê step thiếu] |
| NV-02 | Empty data state | Có UI cho zero data | [có/không] | ✅/❌ | inferred | |
| NV-03 | Network error/timeout | Có UI + retry | [có/không] | ✅/❌ | inferred | |
| NV-04 | Permission denied | Có UI giải thích | [có/không] | ✅/❌ | inferred | |
| NV-05 | Session expired | Có UI cảnh báo | [có/không] | ✅/❌ | inferred | |
| NV-06 | Invalid input | Inline validation | [có/không] | ✅/❌ | inferred | |
| NV-07 | Role-based view | View phù hợp mỗi role | [mô tả] | ✅/❌ | inferred | |
| NV-08 | Data scale | zero/1/few/many/max | [n]/5 state | ✅/❌ | inferred | |
| NV-09 | Trust signal | Bước nhạy cảm có signal | [mô tả] | ✅/❌ | inferred | |
| NV-10 | Confirmation summary | Action không reversible có summary | [mô tả] | ✅/❌ | inferred | |

### Trục Use-case (JTBD Coverage) — [pass]/8 pass = [x]%

| ID | Tiêu chí | Ngưỡng pass | Kết quả thực tế | Pass? | Method | Ghi chú cụ thể |
|----|----------|-------------|-----------------|-------|--------|----------------|
| UC-01 | JTBD rõ ràng | Articulate được | [có/không] | ✅/❌ | inferred | |
| UC-02 | US ≥2, ≤5 | Số US hợp lý | [n] US | ✅/❌ | measured | |
| UC-03 | AC testable | 3–5 AC/US | [mô tả] | ✅/❌ | inferred | |
| UC-04 | US coverage | ≥80% US có UI | [x]% | ✅/❌ | inferred | [US-XX ✅/⚠️/❌] |
| UC-05 | Outcome metrics | Speed/Accuracy/Satisfaction | [mô tả] | ✅/❌ | inferred | |
| UC-06 | Hypothesis có bằng chứng | Cite nguồn | [có/không] | ✅/❌ | inferred | |
| UC-07 | Không US orphan | Mỗi US có UI | [mô tả] | ✅/❌ | inferred | |
| UC-08 | Không UI orphan | Mỗi UI phục vụ US | [mô tả] | ✅/❌ | inferred | |

### Hard Gate — [PASS/FAIL]

| ID | Mục | Ngưỡng | Kết quả | Pass? | Ghi chú |
|----|-----|--------|---------|-------|---------|
| H1 | Contrast WCAG AA | ≥95% text đạt 4.5:1 | [x]% | ✅/❌ | [node vi phạm] |
| H2 | Tap target | ≥95% interactive ≥24×24 | [x]% | ✅/❌ | |
| H3 | Primary CTA | Tồn tại + duy nhất | [có/không] | ✅/❌ | |
| H4 | Empty state | Mọi list có | [x]% | ✅/❌ | |
| H5 | Error state | Mọi form+async có | [x]% | ✅/❌ | |
| H6 | Loading state | Mọi async có | [x]% | ✅/❌ | |
| H7 | Destructive confirm | Có dialog/undo | [x]% | ✅/❌ | |
| H8 | Layer naming | ≤20% mặc định | [x]% | ✅/❌ | |
| H9 | Detached components | ≤5% detach | [x]% | ✅/❌ | |
| H10 | Hardcoded color | ≤10% không bind | [x]% | ✅/❌ | |
| H11 | Auto-layout | ≥80% container | [x]% | ✅/❌ | |

---

## 🔍 Nhận xét tổng quan UI/UX

### Điểm mạnh của thiết kế

[Liệt kê 2–4 điểm thiết kế làm tốt, gắn với US nếu có. Mỗi điểm mạnh cần cụ thể — không khen chung chung.]

1. [Điểm mạnh 1 — tham chiếu US-XX nếu có]
2. [Điểm mạnh 2]

### Vấn đề cốt lõi (tại sao điểm thấp)

[Tóm tắt 2–3 nguyên nhân gốc rễ khiến trục yếu nhất bị kéo xuống. Không lặp lại finding — tổng hợp pattern.]

1. **[Nguyên nhân 1]**: [Giải thích ngắn — VD: "Thiếu feedback loop: thiết kế chỉ cover happy path, không có loading/error/empty → UX và Nghiệp vụ cùng bị kéo"]
2. **[Nguyên nhân 2]**: [Giải thích ngắn]

### Ma trận ưu tiên sửa (Effort × Impact)

| Ưu tiên | Finding | Effort | Impact | Hành động tóm tắt |
|---------|---------|--------|--------|--------------------|
| 🔴 1 | F-XXX | Thấp/TB/Cao | Cao | [1 câu mô tả action] |
| 🔴 2 | F-XXX | Thấp/TB/Cao | Cao | |
| 🟡 3 | F-XXX | ... | ... | |
| ... | ... | ... | ... | |
...
**BẮT BUỘC ghi ít nhất 3 mục từ các nguồn sau:**

1. **Touch target**: Liệt kê các element cần verify ≥44×44pt trên build thật
2. **Accessibility**: Element nào cần a11y label cho screen reader (toast, icon-only button, badge, image)
3. **Token discipline**: Ghi nhận nếu có hardcoded value cần chuyển về design token
4. **State specs chưa có trong design**: Liệt kê state dev cần tự implement (loading duration, animation timing, retry logic, timeout threshold)
5. **Edge case data**: Giới hạn text truncation, số lượng max hiển thị, format đặc biệt (tiền tệ, ngày giờ, số điện thoại)
6. **Non-UI elements**: Ghi rõ element nào trong file Figma là sơ đồ luồng / annotation / wireframe, KHÔNG phải UI cần build
7. **Responsive / adaptive**: Cách ứng xử trên các kích thước màn hình khác nhau nếu applicable
