...
Agent hoạt động trên **Claude Code**, output báo cáo tiếng Việt.
...
> Ngưỡng thực tế per-trục (do mẫu số nhỏ + làm tròn lên): UI ≥9/11 (~82%), UX ≥8/9 (~89%), NV ≥8/10 (80%), UC ≥7/8 (~88%). Chi tiết tại `gate-rules.md`.
...
### 7.1 Tăng độ tin cậy cho trục UX (evidence-driven)

Trục UX dễ bị “miss” nếu chỉ dựa vào screenshot/tên layer. Vì vậy agent có thêm các “scanner scripts” để tạo **evidence nodeId** trước khi kết luận:

- **UX State coverage**: quét `loading/empty/error/success` theo nhiều tín hiệu (tên layer + text + pattern) → giảm miss UX-05/06/07 và hard gate H4/H5/H6  
  Tham chiếu: `scripts/ux-state-scanner.md`
- **Destructive actions**: quét hành động nguy hiểm và đối chiếu **confirmation/undo** → giảm miss UX-10 và hard gate H7  
  Tham chiếu: `scripts/destructive-action-scanner.md`
- **Flow scan**: ưu tiên prototype links để phát hiện **dead-end / escape hatch / continuity**; nếu thiếu prototype thì ghi rõ confidence  
  Tham chiếu: `scripts/ux-flow-scanner.md`
...
Ngoài ra, các scanner cho UX giúp AI codegen/triển khai giảm “mù state”:
- Dễ thấy cần implement state nào (loading/empty/error/success)
- Giảm bỏ sót confirm/undo cho destructive actions
...
- **Measured items càng nhiều → sai lệch càng thấp.** 10 scanner scripts đã nâng tỷ lệ measured lên 18/38 items + 6/11 Hard Gate.
- **Trục UI và UX** có sai lệch thấp nhất (nhiều measured) — đáng tin nhất.
- **Trục Nghiệp vụ** có sai lệch cao nhất (100% inferred) — cần đọc kèm cảnh báo confidence ±10–15% trong báo cáo.
