...
|----|-----|--------|--------|---------|
| H1 | Contrast WCAG AA | ≥95% text element đạt 4.5:1 (normal) hoặc 3:1 (large ≥18pt hoặc 14pt bold) | `measured`* | Đo bằng `scripts/contrast-checker.md` (tính WCAG ratio từ hex). *Fallback `inferred` khi text trên ảnh/gradient phức tạp hoặc có opacity/blend mode |
| H2 | Tap target | ≥95% interactive element đạt 24×24 CSS px (WCAG 2.5.8). Mobile khuyến nghị 44×44pt | `measured`* | Đo bằng `scripts/tap-target-checker.md` (đọc absoluteBoundingBox). *Fallback `inferred` khi cần nhận diện "interactive" từ naming thay vì type |
...
**Tổng hard gate**: 11 items — 6 `measured`* + 5 `inferred`. H1 và H2 mặc định `measured` nhờ scanner scripts; nếu scanner không chạy được (ví dụ thiếu data) → fallback `inferred`. Báo cáo BLOCKED phải nói rõ item nào đo chính xác, item nào ước lượng.
